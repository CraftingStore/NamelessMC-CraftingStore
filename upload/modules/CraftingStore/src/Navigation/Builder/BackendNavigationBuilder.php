        ...
        if (defined('BACK_END')) {
            if ($user->hasPermission(PermissionEnum::DEFAULT)) {
                if ($user->hasPermission(PermissionEnum::SETTINGS)) {
                    ...
                    $navigation->add('craftingstore_remote', $this->craftingStoreLanguage->get('language', LanguageEnum::CRAFTING_STORE), 'https://craftingstore.net/admin', 'top', '_blank', ($order + 0.3), '<i class="nav-icon fas fa-external-link-alt"></i>');
                }
            }
        }
    }
